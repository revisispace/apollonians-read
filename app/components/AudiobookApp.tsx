...
const makeNotification = (title: string, detail: string, target?: ViewId): HeaderNotification => ({
...
export function AudiobookApp() {
  ...
  const pushNotification = (title: string, detail: string, target?: ViewId) => {
    setNotifications((current) => [makeNotification(title, detail, target), ...current].slice(0, 20));
  };
      ...
      try {
        if (await hasLegacyLocalBooks()) {
          ...
          if (claimed > 0 && activeRequest) {
            ...
            pushNotification("Buku lokal dipulihkan", message, "library");
          }
        }
        ...
        if (recovery.recovered > 0 && activeRequest) {
          ...
          setStorageMessage(message);
          pushNotification("Audio berhasil dipulihkan", message, "library");
          ...
          setStorageMessage(message);
          pushNotification("Proses audio masih berjalan", message, "activity");
        }
        ...
        if (activeRequest) {
          const message = error instanceof Error ? error.message : "Data akun gagal dimuat.";
          setStorageMessage(message);
          pushNotification("Data akun gagal dimuat", message, "library");
        }
      }
    };
  ...
  const searchResults = useMemo<HeaderSearchResult[]>(() => {
    ...
    if (!clean) return [];
    return allBooks
      .filter((book) => `${book.title} ${book.author} ${book.category}`.toLowerCase().includes(clean))
      .slice(0, 8)
      .map((book) => ({ id: book.id, title: book.title, subtitle: `${book.author} · ${book.category}` }));
  }, [allBooks, query]);
  ...
  const createdBook = async (book: Book) => {
    ...
    if (normalized.generated && userId) {
      ...
      pushNotification("Audiobook selesai dibuat", `${normalized.title} siap didengarkan.`, "library");
    }
  ...
  const handleLocalDataCleared = () => {
    ...
    pushNotification("Data lokal dihapus", "Seluruh buku, audio, bookmark, dan progres lokal akun ini telah dihapus.", "settings");
  };
  ...
  const openNotification = (notification: HeaderNotification) => {
    ...
    if (notification.target) setActive(notification.target);
  };
    ...
    <div className="app-shell">
      ...
      <div className="app-column">
        <AppHeader
          ...
          onMarkAllNotificationsRead={() => setNotifications((current) => current.map((item) => ({ ...item, unread: false })))}
