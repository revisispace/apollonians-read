...
export function LibriVoxView({ userId }: { userId: string }) {
  ...
  const loadCatalog = useCallback(async () => {
    ...
    setError("");
    try {
      const result = await searchLibriVoxBooks({ query: submittedQuery, field: submittedField, limit: PAGE_SIZE, offset: page * PAGE_SIZE });
  ...
  return <div className="view librivox-view">
    ...
    <section className="librivox-hero"><div><p className="eyebrow">AUDIOBOOK PUBLIC DOMAIN</p><h1>Jelajahi <em>LibriVox.</em></h1><p>Katalog tidak dimuat sekaligus. Cari buku yang dibutuhkan, lalu aplikasi hanya mengambil delapan hasil per halaman.</p></div><Headphones size={48} /></section>
    <form className="librivox-search" onSubmit={(event) => { event.preventDefault(); submitSearch(); }}><Search size={19} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ketik minimal 3 karakter..." /><select value={field} onChange={(event) => setField(event.target.value as LibriVoxSearchField)}><option value="title">Judul</option><option value="author">Penulis</option><option value="genre">Genre</option></select><button disabled={loading}>{loading ? "Mencari…" : "Cari"}</button></form>
    {error && <p className="librivox-error">{error}</p>}

    {!selected ? <>
      {!hasSearched ? <div className="librivox-empty"><Search size={34} /><strong>Cari audiobook LibriVox</strong><p>Katalog baru dihubungi setelah kamu menekan Cari atau Enter, sehingga halaman tetap ringan.</p></div> : loading ? <div className="librivox-loading"><LoaderCircle className="spin" /> Mencari “{submittedQuery}”…</div> : books.length ? <div className="librivox-grid">{books.map((book) => <button key={book.id} className="librivox-card" onClick={() => void openBook(book)}><div className="librivox-cover">{book.thumbnailUrl ? <img src={book.thumbnailUrl} alt="" loading="lazy" /> : <BookOpenText size={38} />}</div><div><small>{book.language} · {book.sectionCount || "?"} bagian</small><strong>{book.title}</strong><span>{book.author}</span><p>{book.totalTime || "Durasi belum tersedia"}</p></div></button>)}</div> : <div className="librivox-empty"><BookOpenText size={34} /><strong>Tidak ada hasil</strong><p>Coba kata kunci atau jenis pencarian lain.</p></div>}
      ...
      <button className="librivox-back" onClick={() => { audioRef.current?.pause(); setSelected(null); }}>← Kembali ke hasil</button>
