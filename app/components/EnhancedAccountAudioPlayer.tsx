...
export function EnhancedAccountAudioPlayer({ book, userId }: { book: Book; userId: string }) {
  ...
  const [audioTexts, setAudioTexts] = useState<string[]>([]);
  ...
  const partRatio = duration > 0 ? Math.min(1, elapsed / duration) : 0;
  ...
  useEffect(() => {
    ...
    return () => { active = false; audio?.pause(); urlsRef.current.forEach(URL.revokeObjectURL); };
  ...
  useEffect(() => {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({ title: book.title, artist: book.author, album: activeChapter?.title ?? `Bagian ${chunk + 1}` });
    navigator.mediaSession.playbackState = playing ? "playing" : "paused";
  }, [activeChapter?.title, book.author, book.title, chunk, hasAudio, playing]);
  ...
  const moveToChunk = (target: number, currentTime = 0, autoplay = false) => {
    ...
    setChunk(target); setElapsed(currentTime); setDuration(0);
    audio.src = urlsRef.current[target]; audio.currentTime = currentTime; audio.playbackRate = speedRef.current; audio.load();
  ...
  return <>
    <section className={`audio-player ${hasAudio ? "" : "is-unavailable"}`} aria-label="Pemutar audio">
      ...
      <button className="player-book player-book-open" disabled={!hasAudio} onClick={() => hasAudio ? setFullOpen(true) : setMessage("Audio belum tersedia")}>
        <BookCover title={book.title} author={book.author} palette={book.palette} /><div><strong>{book.title}</strong><small>{hasAudio ? `${activeChapter?.title ?? `Bagian ${chunk + 1}`} · ${chunk + 1}/${chunks}` : message}</small></div><ChevronDown className="player-expand-icon" size={18} />
      </button>
